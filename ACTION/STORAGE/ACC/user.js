...
exports.clearAllUser = (_dir) => {
    Object.keys(_dir).forEach((i) => {
        ...
        fs.writeFileSync('./ACTION/STORAGE/ADD/user.json', JSON.stringify(_dir))
    })
...
exports.AddHit = function(sender, _db){
    ...
    if (found !== false) {
        _db[found].hit += 1;
        fs.writeFileSync('./ACTION/STORAGE/ADD/user.json',JSON.stringify(_db));
    }
}
...
exports.addLevelingXp = (sender, amount, _level ) => {
	...
	if (position !== false) {
		...
		fs.writeFileSync('./ACTION/STORAGE/ADD/user.json', JSON.stringify(_level))
	}
}
